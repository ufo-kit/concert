...
from typing import List, AsyncIterator, Tuple, Dict, Callable
import numpy as np
...
class RotationAxisEstimator(TangoRemoteProcessing):
    ...
    _implementation: Callable[[AsyncIterator[ArrayLike], int, int, float], None]
    ...
    def estimation_algorithm(self, est_algo: int) -> None:
        ...
        if self._estimation_algorithm == EstimationAlgorithm.MT_SEGMENTATION:
            self._implementation = self._estimate_mt_segmentation
        elif self._estimation_algorithm == EstimationAlgorithm.MT_HOUGH_TRANSFORM:
            self._implementation = self._estimate_mt_hough_transform
        elif self._estimation_algorithm == EstimationAlgorithm.OPT_GRADIENT_DESCENT:
            self._implementation = self._estimate_opt_gd
        else:
            raise RuntimeError("unsupported estimation algorithm")
    ...
    async def estimate_center_of_rotation(self, args: Tuple[float]) -> None:
        ...
        await self._process_stream(self._implementation(self._receiver.subscribe(),
    ...
    async def _estimate_mt_hough_transform(self, producer: AsyncIterator[ArrayLike],
                                           wait_window: int, check_window: int,
                                           err_threshold: float) -> None:
        """
        Estimates the center of rotation with marker tracking using nonlinear polynomial fit and
        ...
        self.info_stream("%s: Meta information for rotation axis estimation with Hough Transform",
        ...
        # For Hough circle transform we perform flat field correction with absorptivity.
        ...
        async for projection in ffc(producer):
            edges: ArrayLike = smf.canny(projection[:crop_vertical, crop_left:crop_right])
            ...
            centroids: List[List[int]] = [[center_y, center_x] for center_y, center_x in zip(
                coords_y, coords_x)]
            ...
            # greater or equal to the expected number of markers. In practice, this is an edge case
            # because it might not always be true.
            ...
            marker_centroids.append(np.array(centroids)[:, 1])
    ...
    async def _estimate_mt_segmentation(self, producer: AsyncIterator[ArrayLike], wait_window: int,
        ...
        estm_rot_axis: List[float] = []
        event_triggered = False
        crop_left, crop_right, crop_vertical, num_markers, _ = self._marae
        self.info_stream("%s: Meta information for rotation axis estimation with segmentation",
                         self.__class__.__name__)
        self.info_stream("crop: [:%d, %d: %d], num_markers: %d",
                         crop_vertical, crop_left, crop_right, num_markers)
        # For Hough circle transform we perform flat field correction with absorptivity.
        ffc = FlatCorrect(dark=self._dark, flat=self._flat, absorptivity=True)
        # Process all projections for potential marker centroids but run curve-fit conditionally.
        async for projection in ffc(producer):
            ...
            centroids: List[ArrayLike] = [region.centroid for region in regions]
            # We sort marker centroids w.r.t vertical coordinate to be consistent with the markers
            # during curve_fit.
            centroids = sorted(centroids, key=lambda centroid: centroid[0])
            # TODO: Here we implicitly assume that the detected number of regions would always be
                ...
                if projection_count % self._estm_offset == 0:
                    try:
                        ...
                        for mix in range(num_markers):
                            ...
                            rot_axes.append(params[0])
                        self.info_stream(
                            "%s: Preliminary rotation axis with projections [%d/%d]: %f",
                            self.__class__.__name__,
                            projection_count + 1,
                            self._num_radios,
                            np.median(rot_axes)
    ...
    async def _estimate_opt_gd(self, producer: AsyncIterator[ArrayLike], wait_window: int,
                               check_window: int, err_threshold: float) -> None:
        """Estimates the center of rotation with gradient descent optimization"""

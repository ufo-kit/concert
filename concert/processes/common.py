...
class AlignmentContext:
    ...
    - use binary mask to compute sample offsets
    """
    devices: AlignmentDevices
    pixel_size_um: Quantity
    max_iterations: int = 50
    pixel_sensitivity: int = 1
    init_gain: float = 1.0
    ceil_gain: float = 5.0
...
async def acquire_single(ctx: AcquisitionContext) -> ArrayLike:
    """
...
async def align_rotation_stage_ellipse_fit(
    ...
    for iteration in range(align_ctx.max_iterations):
        logger.debug("=" * 3 * len(f"Start: {func_name}"))
        acq_consumers = [functools.partial(extract_ellipse_points, radius=acq_ctx.sphere_radius),
                         frames_result]
...
async def get_sample_shifts(
        acq_ctx: AcquisitionContext, align_ctx: AlignmentContext,  align_state: AlignmentState,
...
async def init_alignment_state(
        acq_ctx: AcquisitionContext, align_ctx: AlignmentContext) -> AlignmentState:
    """
...
async def move_relative(
        motor: Motor_T, move_by: Quantity, align_ctx: AlignmentContext,
    ...
    while offset_obd > align_ctx.pixel_err_eps or offset_pbd > align_ctx.pixel_err_eps:
        ...
        if offset_obd > align_ctx.pixel_err_eps:
            offset_obd = await _move_corrective(
                motor=align_ctx.devices.align_motor_obd, offset=offset_obd, start_ang_deg=0 * q.deg)
        ...
        if offset_pbd > align_ctx.pixel_err_eps:
            offset_pbd = await _move_corrective(
                motor=align_ctx.devices.align_motor_pbd, offset=offset_pbd, start_ang_deg=90 * q.deg)
    logger.debug(f">> after: offset_obd = {offset_obd} offset_pbd = {offset_pbd}")


async def offset_from_projection_center(
        acq_ctx: AcquisitionContext, align_ctx: AlignmentContext) -> Tuple[float, float]:
    """
    ...
    """
    frame: ArrayLike = await acquire_single(ctx=acq_ctx)
    ref_img: ArrayLike = make_binary(frame, sigma=acq_ctx.sigma) if align_ctx.use_binary else frame
...
async def align_rotation_stage_comparative(
    ...
    align_state: AlignmentState = await init_alignment_state(acq_ctx=acq_ctx, align_ctx=align_ctx)
